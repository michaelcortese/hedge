...
def test_settlement_booking_updates_daily_pnl(monkeypatch, tmp_path):
    import hedge.runner as runner_mod
    t = "KXHIGHCHI-26JUN29-T80"
    client = _RunnerClient()
    client.results[t] = "yes"   # market settled YES
    r = _runner(client, tmp_path, monkeypatch, t)
    # Write a prior acted-on decision log for this ticker so _book_settlements sees it.
    runner_mod.LIVE_DIR.mkdir(parents=True, exist_ok=True)
    import json as _json
    row = {"ticker": t, "action": "buy", "side": "yes", "price_cents": 55,
           "count": 4, "prob": 0.8, "ts": "2026-06-29T12:00:00+00:00"}
    (runner_mod.LIVE_DIR / "decisions_2026-06-29.jsonl").write_text(_json.dumps(row) + "\n")
    r.run_cycle()
    # Won YES at 0.55 -> ~ +0.43/contract * 4 contracts, booked into today's P&L.
    assert r.state.realized_today() > 1.0
